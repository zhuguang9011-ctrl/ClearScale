...
## v1.2.0-beta.1 局部材质精修（实验功能）

新增产品框选、丝绸 PLA / 黑色注塑 / 通用材质预设、0.15–0.25 低强度分块重绘。
此功能需要**另行安装并启动本机 ComfyUI、Ultimate SD Upscale 节点和 SD1.5 模型**，不包含在基础高清引擎中。
选区外不参与生成，选区内可能改变结构与颜色，需检查结果。尚未完成真实 GPU 出图验证。
详见 [首次配置与使用指南](REFINE_GUIDE.md)。原有基础高清模式仍可直接使用。
